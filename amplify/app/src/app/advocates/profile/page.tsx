...
const Profile = () => {
  const { data: posts, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await axios.get(`/api/advocate/post/profile`);
      return response.data.lawyerPosts as Post[];
    },
    staleTime: 1000 * 60 * 5,
  });

  const { data: lawyerDetails, isLoading: isFetchingLawyerDetails } = useQuery({
    queryKey: ["lawyerDetails"],
    queryFn: async () => {
      const response = await axios.get(`/api/advocate`);
    ...
    posts.length !== 0 && (
      <div className="min-h-screen flex">
        <div className="p-6 max-w-5xl w-full mx-auto space-y-6">
          <h1 className="text-3xl font-bold mb-6 text-center">Profile</h1>
          {!isFetchingLawyerDetails && (
            <div>
              <h2 className="text-lg font-semibold">{lawyerDetails?.name}</h2>
              <p className="text-gray-300 text-sm">{lawyerDetails?.email}</p>
            </div>
          )}
          <div className="space-y-6">
            {posts.map((post) => {
              ...
              if (post.genre === "Constitution Facts") {
                genreIcon = (
                  <Landmark className="inline-block mr-2" size={18} />
                );
